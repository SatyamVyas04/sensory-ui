export const metadata = {
  title: "Installation",
  description:
    "Install sensory-ui and add semantic sound to your shadcn/ui components.",
};
...
2. Present a prompt to choose a **sound pack**
3. Copy all files into your project
4. Print a short post-install message with the next steps

The registry is served directly from the public GitHub repository via `registry.json` — no custom registry server or URL required.
...
All packs use the same role names and registry structure. Swapping packs later is a one-line change in `sensory.config.js` or passing `config={{ theme: "arcade" }}` to `SensoryUIProvider`.

<Callout type="info">
  Every pack uses the same role names and structure. You can switch at any time
  by changing the `theme` value.
</Callout>
...
Audio is generated **programmatically** via the Web Audio API inside `sensory-ui/sounds/` — no files in `public/`, no asset serving, no base64 blobs. The only file outside that folder is `sensory.config.js` at the project root (optional, can be deleted to use defaults).
