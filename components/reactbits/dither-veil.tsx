...
const fitScale = (w: number, h: number, iw: number, ih: number, contain: boolean) => {
  const ratio = w / h / (iw / ih);
  if (contain) return ratio > 1 ? [ratio, 1] : [1, 1 / ratio];
  return ratio > 1 ? [1, 1 / ratio] : [ratio, 1];
};

const wanderAt = (t: number, w: number, h: number) => [
...
const measureEdge = (context: CanvasRenderingContext2D, image: HTMLImageElement) => {
  ...
  const spread =
    matte.reduce(
      (total, mean, c) => total + Math.sqrt(Math.max(0, squares[c] / count - mean * mean)),
      0
    ) / 3;
...
interface Settings {
  fit: "contain" | "cover";
  pattern: DitherVeilProps["pattern"];
  pixelSize: number;
  levels: number;
  palette: "duotone" | "rgb";
  inkColor: string;
  ...
  useEffect(() => {
    ...
    const createMask = (w: number, h: number) =>
      new RenderTarget(gl, {
        ...
        height: h,
        depth: false,
        type: floatMask ? GL_HALF_FLOAT : GL_UNSIGNED_BYTE,
        internalFormat: floatMask ? GL_RGBA16F : GL_RGBA,
        minFilter: gl.LINEAR,
        magFilter: gl.LINEAR,
      });
    ...
    const updateDiffusion = (cell: number, s: Settings) => {
      ...
      const grade = (v: number) =>
        Math.pow(Math.min(1, Math.max(0, (v - 0.5) * s.contrast + 0.5 + s.brightness)), 1.6);
      const pixels = samplerContext.getImageData(0, 0, cols, rows).data;
      const kernel = KERNELS[s.pattern ?? ""];
      diffusedTexture.image = diffuse(pixels, cols, rows, kernel ?? [], s.levels, s.palette === "rgb", grade);
    ...
    const frame = (now: number) => {
      ...
      bursts.forEach((b, i) => {
        ...
        const reach =
          Math.hypot(Math.max(b.x, width - b.x), Math.max(b.y, height - b.y)) +
      ...
      let patternIndex = ORDERED[s.pattern ?? ""];
    ...
    document.documentElement.addEventListener("pointercancel", onLeave, { passive: true });
